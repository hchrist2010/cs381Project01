...
sub build_bigrams{
	...
	foreach $title(@tracks){
		...
		for(my $i = 0; $i < $#wordArray; $i++){
			...
			if(!exists($counts{$wordArray[$i]}{$wordArray[$i + 1]})) {
				$counts{$wordArray[$i]}{$wordArray[$i + 1]} = 1;
			}
			else{
				$counts{$wordArray[$i]}{$wordArray[$i + 1]} += 1;
			}
			#print("$wordArray[$i] $wordArray[$i+1] $counts{$wordArray[$i]}{$wordArray[$i + 1]}\n");
		}
	}
	#return %counts;
